...
<div class="qlquicklink" id="module<?php echo $module->id ?>">
    <ul>
        <?php foreach ($buttons as $button) : ?>
            <li>
                <a href="<?= $button->getLink() ?>" title="<?= $button->getLabel() ?>" type="button" id="sidebarCollapse" class="btn btn-primary">
                    <?php if (!empty($button->getFa())): ?><i class="fa fa-<?= $button->getFa() ?>"></i><?php endif; ?>
                    <?php if (!empty($button->getImage())): ?><img src="<?= $button->getImage() ?>" alt="<?= Text::_('MOD_QLQUICKLINK_LINKTO') ?> <?= $button->getLabel() ?>" /><?php endif; ?>
