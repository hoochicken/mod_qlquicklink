...
$fas = array_filter($fas, fn($item) => str_contains((string) $item, 'i class="fa') || str_contains((string) $item, 'muted'));
array_walk($fas, function(&$item) {
    if (str_contains((string) $item, 'i class')) {
        $item = trim(substr((string) $item, strrpos((string) $item, ' ')));
    }
    if (str_contains((string) $item, 'text-muted')) {
        $item = trim(substr((string) $item, strpos((string) $item, '#') + 1));
...
foreach ($fas as $key => $value) {
    $type = str_contains((string) $value, 'fa') ? 'class' : 'glyph';
    if ('class' === $type) {
        $fasNew[$key] = $value;
        $keyPrev = $key;
        continue;
...
$fas = array_filter($fasNew, fn($item) => str_starts_with((string) $item, 'fa'));
$resultGlyph = [];
foreach ($fas as $value) {
    $info = array_values(array_filter(explode(' ', (string) $value)));
    if (!isset($info[1])) {
        continue;
    }
...
$raw = file_get_contents($src);
$fas = explode("\r\n\r\n", $raw);
$fas = array_filter($fas, fn($item) => str_contains((string) $item, 'content:') && !str_contains((string) $item, '/* Font Awesome uses the Unicode Private'));
